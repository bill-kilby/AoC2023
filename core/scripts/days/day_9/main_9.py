...
def RunSolution():
    """
    Executes and displays the solutions for Day 9.

    This function runs two separate functions: SilverSolution and GoldSolution,
    which compute the solutions for the silver and gold stars of Day 9, respectively.
    It then prints out these solutions.
    """
    print("Running Day 9 Solution...")
    ...
    print(f"The Silver Solution to Day 9 -> {silver}")
    print(f"The Golden Solution to Day 9 -> {golden}")
...
def FileImport():
    ...
    try:
        ...
        scriptDirectory = scriptDirectory.replace("\scripts\days\day_9", "") # Removing up to core.
        scriptDirectory = scriptDirectory+"\inputs\day_9\silver.txt" # Adding the file import.
        with open(scriptDirectory) as daySpec:
            #TODO: Process the file import here depending on what comes.
            processedSilver = 0
    except:
        raise FileNotFoundError(f"Day 9's silver import file cannot be found!")
    # Getting the gold solution.
    try:
        scriptDirectory = os.path.dirname(__file__) # Finding the current directory string.
        scriptDirectory = scriptDirectory.replace("\scripts\days\day_9", "") # Removing up to core.
        scriptDirectory = scriptDirectory+"\inputs\day_9\gold.txt" # Adding the file import.
        with open(scriptDirectory) as daySpec:
            #TODO: Process the file import here depending on what comes.
            processedGold = 0
    except:
        raise FileNotFoundError(f"Day 9's gold import file cannot be found!")
