...
def RunSolution():
    """
    Executes and displays the solutions for Day 23.

    This function runs two separate functions: SilverSolution and GoldSolution,
    which compute the solutions for the silver and gold stars of Day 23, respectively.
    It then prints out these solutions.
    """
    print("Running Day 23 Solution...")
    ...
    print(f"The Silver Solution to Day 23 -> {silver}")
    print(f"The Golden Solution to Day 23 -> {golden}")
...
def FileImport():
    ...
    try:
        ...
        scriptDirectory = scriptDirectory.replace("\scripts\days\day_23", "") # Removing up to core.
        scriptDirectory = scriptDirectory+"\inputs\day_23\silver.txt" # Adding the file import.
        with open(scriptDirectory) as daySpec:
            #TODO: Process the file import here depending on what comes.
            processedSilver = 0
    except:
        raise FileNotFoundError(f"Day 23's silver import file cannot be found!")
    # Getting the gold solution.
    try:
        scriptDirectory = os.path.dirname(__file__) # Finding the current directory string.
        scriptDirectory = scriptDirectory.replace("\scripts\days\day_23", "") # Removing up to core.
        scriptDirectory = scriptDirectory+"\inputs\day_23\gold.txt" # Adding the file import.
        with open(scriptDirectory) as daySpec:
            #TODO: Process the file import here depending on what comes.
            processedGold = 0
    except:
        raise FileNotFoundError(f"Day 23's gold import file cannot be found!")
